import XCTest
@testable import Insulog

@MainActor
final class InsulogTests: XCTestCase {
    var store: Store!
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(InsulationEntryEntry(title: "New One"))
        XCTAssertEqual(store.entries.count, before + 1)
    }

    func testDeleteEntryDecreasesCount() {
        store.add(InsulationEntryEntry(title: "ToDelete"))
        let before = store.entries.count
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        store.isPro = false
        store.entries = (0..<Store.freeLimit).map { InsulationEntryEntry(title: "Item \($0)") }
    ...
    func testUpdateEntryChangesTitle() {
        store.add(InsulationEntryEntry(title: "Original"))
    ...
    func testDeleteAtOffsetsRemovesCorrectEntry() {
        store.entries = []
        store.add(InsulationEntryEntry(title: "A"))
        store.add(InsulationEntryEntry(title: "B"))
